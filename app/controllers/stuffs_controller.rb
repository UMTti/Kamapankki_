class StuffsController < ApplicationController
  ...
  def edit
   ...
   def borrowrequest
     ...
     @message = Message.new
     @message.content = "Someone wants to borrow your item!"
     @message.sender_id = current_user.id
     @message.stuff_id = @stuff.id
     @message.save
     @stuff.user.messages << @message

     respond_to do |format|
      if @message.save
        format.html { redirect_to root_url, notice: 'Borrow request sent!' }
  ...
  def create
    ...
    @stuff.borrowed = false

    respond_to do |format|
      if @stuff.save
        format.html { redirect_to root_url, notice: 'Stuff was successfully created.' }
        format.json { render :show, status: :created, location: @stuff }
      else
  ...
  private
    ...
    def stuff_params
      params.require(:stuff).permit(:name, :user_id, :borrowed)
    end
end
